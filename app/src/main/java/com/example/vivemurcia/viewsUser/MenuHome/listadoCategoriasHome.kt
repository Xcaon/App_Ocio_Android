package com.example.vivemurcia.viewsUser.MenuHome
...
@Composable
fun listadoCategoriasHome(navController: NavController, categoriaNombre: String) {

    val homeViewModel: HomeViewModel = hiltViewModel<HomeViewModel>()
    val viewModelDetalle : ViewModelDetalle = hiltViewModel<ViewModelDetalle>()

    val catEnum = EnumCategories.entries.find { it.name.equals(categoriaNombre, ignoreCase = true) }

    // Obtenemos las actividades de Firestore
    val actividades: List<Actividad> by homeViewModel.actividadesPorCategoria[catEnum]!!.collectAsState()
